...
export class HederaEngine extends CurrencyEngine {
  ...
  constructor(
    ...
    this.otherMethods = {
      ...
      submitActivationPayment: async (txn) => {
  ...
  async checkAccountCreationStatus() {
    ...
    if (accountId == null && paymentSubmitted && activationRequestId != null) {
      try {
        ...
        if (_optionalChain([e, 'optionalAccess', _ => _.message]) === 'ErrorAccountActivation') throw e
      }
    }
  ...
  async updateBalance() {
    ...
    const hbarBalance = await new hedera.AccountBalanceQuery()
      ...
      .execute(this.client)
    const nativeBalance = hbarBalance.asTinybar().toString()
